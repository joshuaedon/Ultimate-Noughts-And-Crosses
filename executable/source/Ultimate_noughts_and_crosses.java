...
// Update size values when the window size is changed
 ...
 public void highlightAvailable() {
  ...
  if(curSubgrid.x < 0) {
    ...
    detailedGame[PApplet.parseInt(curSubgrid.x)][PApplet.parseInt(curSubgrid.y)].highlightAvailable(curSubgrid, cellSize, margin, highlightIntensity);
  }
}
...
class SmallGrid {
  ...
  SmallGrid(int n) {
   ...
   public void highlightAvailable(PVector superDims, float cellSize, int margin, float highlightIntensity) {
    noStroke();
    int white = color(255);
    int blue = color(207, 238, 250);
