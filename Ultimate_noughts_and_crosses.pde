...
void highlightAvailable() {
  ...
  //float highlightIntensity = 2 * abs((millis() % highlightPulseTime) / highlightPulseTime - 0.5);
  float highlightIntensity = (sin(millis() / highlightPulseTime) + 2) / 4;
  
  if(curSubgrid.x < 0) {
    for(int i = 0; i < n; i++) {
      for(int j = 0; j < n; j++) {
        PVector superDims = new PVector(i, j);
        detailedGame[i][j].highlightAvailable(superDims, cellSize, margin, highlightIntensity);
      }
    }
  } else {
    detailedGame[int(curSubgrid.x)][int(curSubgrid.y)].highlightAvailable(curSubgrid, cellSize, margin, highlightIntensity);
  }
}
